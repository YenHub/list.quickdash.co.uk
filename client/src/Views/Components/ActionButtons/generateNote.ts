...
export const properCase = (str: string): string =>
  str.replace(
    /\w\S*/g,
    (txt: string) =>
      txt.charAt(0).toUpperCase() + txt.slice(0, 1).toLowerCase(),
  )

const getWords = (count?: number): string =>
  properCase(
    random(100) > 50 ? faker.word.words(count) : faker.lorem.words(count),
  )

const getWord = (): string =>
  properCase(random(100) > 40 ? faker.word.words() : faker.lorem.words())

const generatePrimary = (): string | undefined => {
  const testVal = random(100)
  if (testVal < 10) return
  if (testVal < 60) return getWords()

  return properCase(faker.word.words())
}

const generateSecondary = (): string | undefined => {
  const testVal = random(100)

  if (testVal < 10) return testVal < 4 ? `> ${getWords(random(15))}` : ''
  ...
  if (testVal < 40) {
    if (random(50) > 25)
      return (
        ...
        `- ${getWords()}\n\n` +
        '\n\n---\n\n' +
        `### ${properCase(faker.word.words(random(5)))}\n\n` +
        `- ${getWords()}\n\n` +
        `${random(10) > 5 ? '' : ' '}- ${getWords()}\n\n` +
        `- ${getWords()}`
      )
