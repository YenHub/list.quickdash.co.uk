...
export const ShareButton: FC = () => {
  ...
  const { setSyncSettings, noteState, webId, setNotes } = useAppStore(
    state => ({
    ...
    /* New list to sync */
      await syncNewList()
        .then(async res => {
          ...
          await handleCreateSuccess({ version, webId, syncSequence, listItems })
        })
        .catch(e => {
          errorLog(e as string)
          handleCreateFail()
        })
    // IGDev: Show Link Modal
  }
  ...
  if (saved) return <Checkmark size="35px" color={theme.palette.primary.main} />
