...
const CreateNoteModal: React.FC = () => {
  ...
  const handleClose = (): void =>
    setModalState({ open: false, editingNoteId: null })
  ...
  const editExistingNote = async (editingNoteId: string): void => {
    const newNote = {
      id: editingNoteId,
      primary,
      secondary,
  ...
  const createNote = async (evt: FormEvent | MouseEvent): Promise<void> => {
    handleClose()
    evt.preventDefault()
    switch (true) {
      ...
      case editingNoteId && editingNoteId !== '':
        ...
        return editExistingNote(editingNoteId)
      case !!noteState?.length:
        ...
        await setNotes([
          { id: getUniqueId(noteState), primary, secondary },
          ...noteState,
        ])
