...
interface AppActions {
  ...
  /* Note actions */
  setNotes: (notes: NoteItem[]) => Promise<void>
...
const initialSettingsState: SettingState = {
  ...
  colours: JSON.parse(
    getStringSetting('colours') ?? '{}',
  ) as SettingState['colours'],
}
...
export const useAppStore = create<AppStore>()((set, get) => ({
  ...
  setNotes: async notes => {
    await noteClient.setNotes(notes)
    set(state => ({ notes: { ...state.notes, noteState: notes } }))
  },

  deleteNote: async index => {
    const newNotes = [...get().notes.noteState]
    newNotes.splice(index, 1)
    await noteClient.setNotes(newNotes)

    set(state => ({ notes: { ...state.notes, noteState: newNotes } }))
  },
  ...
  setVersion: version =>
    set(state => ({ settings: { ...state.settings, version } })),
